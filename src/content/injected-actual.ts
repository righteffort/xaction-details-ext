...
function connectToActual() {
  ...
  while (fiber && steps < 50) {
    ...
    if (p) {
      ...
      if (
        p.transactions &&
        typeof p.onSave === "function" &&
        Array.isArray(p.payees) &&
        typeof p.onAdd === "function"
      ) {
        return p;
      }
    }
  ...
  if (command === "IMPORT_TRANSACTION") {
    ...
    if (payload.imported_id) {
      const isDuplicate = existingTransactions.some(
        // eslint-disable-next-line
        (t: any) => t.imported_id === payload.imported_id,
      );
      if (isDuplicate) {
        console.log(
          `Bridge: Skipping duplicate import (${payload.imported_id})`,
        );
        return;
      }
    }
    ...
    try {
      ...
      if (!payeeId)
        throw new Error(
          `Could not resolve payee ID for '${payload.payee_name}'`,
        );
