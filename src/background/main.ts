...
browser.runtime.onMessage.addListener((msg: any, sender: any) => {
  ...
  if (
    msg.action === "RUN_POC_IMPORT_ONE" ||
    msg.action === "RUN_POC_IMPORT_TWO"
  ) {
  ...
  transactions.forEach((tx) => {
    ...
    if (tx.notes.includes("extension test updateme")) {
      ...
      const updatedTx = {
        ...
        notes: tx.notes.replace(
          "extension test updateme",
          "extension test update complete",
        ),
      };
    ...
    if (tx.notes.includes("extension test split me")) {
      ...
      const updatedTx = {
        ...
        notes: tx.notes.replace(
          "extension test split me",
          "extension test split complete",
        ),
        subtransactions: [
          {
            id: crypto.randomUUID(),
